  ...
  <a-spin :spinning="loading">
    ...
    <div class="search_warp" v-bind="state.formWarp">
      <a-form name="queryTableForm" :label-col="{ span: 10 }" :wrapper-col="{ span: 14 }" :inline="true" ref="formRef"
        :model="formInline" class="form" v-bind="state.formProps">
        <div class="search_from_warp">
          <a-row class="search_from_left" :gutter="[24, 24]">
            <a-col v-for="(field, index) in state.fields" :key="index" :span="8">
              <a-form-item :name="field.name" :label="field.label" :rules="field.rules || []">
                ...
                <a-range-picker v-else-if="field.field.type === 'rangepicker'" v-model:value="formInline[field.name]"
                  type="daterange" @keydown.enter="onSubmit()" v-bind="field.field.props" />
                <a-select v-else-if="field.field.type === 'select'" v-model:value="formInline[field.name]"
                  @keydown.enter="onSubmit()" v-bind="field.field.props">
                ...
                <a-checkbox-group v-else-if="field.field.type === 'checkbox'" v-model:value="formInline[field.name]"
                  @keydown.enter="onSubmit()" v-bind="field.field.props">
                  <template v-if="typeof field.source === 'function'">
                ...
                <RenderH @keydown.enter="onSubmit()" v-model:value="formInline[field.name]" v-else v-bind="field.field">
                </RenderH>
              </a-form-item>
            </a-col>
          </a-row>
          <div class="search_actions">
            <a-form-item>
              <a-space>
                <a-button type="primary" @click="onSubmit()" v-bind="state.actionSubmitProps">查询
                </a-button>
                <a-button @click="resetForm(formRef)" v-bind="state.actionCancelProps">重置</a-button>
              </a-space>
            </a-form-item>
          </div>
        </div>
      ...
      <div class="actions_warp_left">
        <a-space>
          <a-button v-for="(btn, index) in state.actions || []" :key="index" @click="btn.action(selectProps, onSubmit)"
            :disabled="btn.isDisabled && selectProps && selectProps.length === 0" :type="btn.type" v-bind="btn.props">
            {{
                btn.text
            }}
          </a-button>
    ...
    <div class="table_warp" v-bind="state.tableWarp">
      <a-table class="table_content" bordered :data-source="pages.data"
        ...
        ref="queryTableRef" :pagination="false" :locale="{
          emptyText: '暂无数据'
        }" @resize-column="handleResizeColumn" :highlightCurrentRow="true" @selection-change="handleSelectionChange"
      ...
      <a-pagination class="page" size="small" :total="pages.total" show-size-changer show-quick-jumper
        ...
        v-model:pageSize="pages.pageSize" @change="currentChangeAndsizeChange" v-bind="state.paginationProps">
